...
def _cutoff(window_minutes=480):
    ...
    return (datetime.now(timezone.utc) - timedelta(minutes=window_minutes)).strftime('%Y-%m-%dT%H:%M:%SZ')
...
async def get_metrics(store_id: str, window_minutes: int = 480, request: Request = None):
    ...
    try:
        ...
        row = conn.execute('''
            ...
            WHERE store_id=? AND event_type='ENTRY' AND is_staff=0 AND timestamp>=?
        ...
        converted = set()
        for txn in txns:
            win_start = (datetime.strptime(txn['timestamp'], '%Y-%m-%dT%H:%M:%SZ')
            ...
            ''', (store_id, win_start, txn['timestamp'])).fetchall()
            for r in rows:
        ...
        zones = [ZoneMetric(zone_id=r['zone_id'], visit_count=r['visits'],
                            avg_dwell_ms=r['avg_dwell'] or 0.0) for r in zone_rows]
        if zones:
        ...
        log.info(json.dumps({'trace_id': trace_id, 'endpoint': f'/stores/{store_id}/metrics',
                             'latency_ms': round(ms, 2), 'status_code': 200}))
        return StoreMetrics(store_id=store_id, unique_visitors=unique_visitors,
                            conversion_rate=round(conversion_rate, 4),
                            avg_dwell_ms=round(avg_dwell, 1),
                            queue_depth=queue_depth,
                            abandonment_rate=round(abandonment_rate, 4),
                            zones=zones, window_minutes=window_minutes,
...
async def get_funnel(store_id: str, window_minutes: int = 480):
    ...
    try:
        ...
        zone_vis = set(r['visitor_id'] for r in conn.execute('''
            SELECT DISTINCT visitor_id FROM events
            WHERE store_id=? AND event_type='ZONE_ENTER' AND is_staff=0 AND timestamp>=?
        ...
        purchased_vis = set()
        for txn in txns:
            win = (datetime.strptime(txn['timestamp'], '%Y-%m-%dT%H:%M:%SZ')
                   - timedelta(minutes=5)).strftime('%Y-%m-%dT%H:%M:%SZ')
            for r in conn.execute('''
                SELECT DISTINCT visitor_id FROM events
                WHERE store_id=? AND zone_id LIKE '%BILLING%'
                AND is_staff=0 AND timestamp BETWEEN ? AND ?
            ''', (store_id, win, txn['timestamp'])).fetchall():
